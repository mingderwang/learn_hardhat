...
contract ERC20_Fountain {
    ...
    event Transfer(address indexed from, address indexed to, uint256 tokensTransferred);
    // ERC20 compliance
    ...
    event Approval(address indexed tokenOwner, address indexed approvedAddress, uint256 newAllowance);
    constructor(uint8 _decimals, string memory _name, string memory _symbol) {
        decimals = _decimals;
        name = _name;
        symbol = _symbol;
    }  
    /// @notice transfers tokens from message sender to another address
    function transfer(address toAddress, uint256 amountTokens) external returns (bool) {
        require( (amountTokens <= tokenBalanceLedger_[ msg.sender ]), "ERC20: Token balance is insufficient for transfer action." );
    ...
    function approve(address approvedAddress, uint256 amountTokens) external returns (bool) {
    ...
    function increaseAllowance(address approvedAddress, uint256 amountTokens) external returns (bool) {
    ...
    function decreaseAllowance( address approvedAddress, uint256 amountTokens) external returns (bool) {
        uint256 pastAllowance = allowance[msg.sender][approvedAddress];
    ...
    modifier checkTransferApproved(address fromAddress, uint256 amountTokens) {
        if (fromAddress != msg.sender){
            require( allowance[fromAddress][msg.sender] <= amountTokens, "ERC20: Transfer not authorized -- allowance insufficient." );
        }
        _;
    } 
    /// @notice transfers tokens from one address to another
    function transferFrom(address fromAddress, address toAddress, uint256 amountTokens)
        checkTransferApproved(fromAddress , amountTokens) external returns (bool) {
        // make sure sending address has requested tokens
        require( (amountTokens <= tokenBalanceLedger_[ fromAddress ]), "ERC20: Transfer not allowed - insufficient funds available." );
    ...
    function mint(address userAddress, uint256 amountTokens) internal {
        require( userAddress != address(0), "ERC20: cannot mint tokens for zero address." );
        totalSupply = totalSupply.add(amountTokens);
        tokenBalanceLedger_[userAddress] = tokenBalanceLedger_[userAddress].add(amountTokens);
    ...
    function burn(address userAddress, uint256 amountTokens) internal {
        require( userAddress != address(0), "ERC20: cannot burn tokens for zero address." );
        require( amountTokens <= tokenBalanceLedger_[userAddress], "Insufficient funds available." );
        tokenBalanceLedger_[userAddress] = tokenBalanceLedger_[userAddress].sub(amountTokens);
    ...
    function transferInternal(address fromAddress, address toAddress, uint256 amountTokens) internal {
        require(fromAddress != address(0), "ERC20: cannot transfer from zero address");
        require(toAddress != address(0), "ERC20: cannot transfer to zero address");
        tokenBalanceLedger_[fromAddress] = tokenBalanceLedger_[fromAddress].sub(amountTokens);
        tokenBalanceLedger_[toAddress] = tokenBalanceLedger_[toAddress].add(amountTokens);
